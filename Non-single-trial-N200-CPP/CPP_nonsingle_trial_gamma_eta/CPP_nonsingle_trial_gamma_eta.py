...
def prior(batch_size):
    ...
    # tau ~ U(0.1, 1.0)
    # gamma ~ ~ U(.5, 4) # is 1/lambda
    # Eta ~ U(0.0, 1.0)
    n_parameters = 6
    p_samples = np.random.uniform(low=(0.0, 0.5, 0.1, 0.1, 0.05, 0.01),
                                  high=(3.0, 2.0, 0.9, 1.0, 4.0, 2.0), size=(batch_size, n_parameters))
    return p_samples.astype(np.float32)

@njit
def diffusion_trial(drift, boundary, beta, ndt, gamma, eta, dc=1.0, dt=.005):
    ...
    # CPP slope
    cpp = np.random.normal(gamma*drift, eta)
...
def diffusion_condition(params, n_trials):
    ...
    drift, boundary, beta, ndt, gamma, eta = params
    choicert = np.empty(n_trials)
    cpp = np.empty(n_trials)
    for i in range(n_trials):
        choicert[i], cpp[i] = diffusion_trial(drift, boundary, beta, ndt, gamma, eta)
    return choicert, cpp
...
trainer = ParameterEstimationTrainer(
    ...
    checkpoint_path="../checkpoint/CPP_nonsingle_trial_gamma_eta"
)
...
true_vs_estimated(true_params, param_means, ['drift', 'boundary', 'beta', 'ndt', 'gamma', 'eta'], filename="../true_vs_estimate/CPP_nonsingle_trial_gamma_eta")
