...
@njit
def diffusion_trial(drift, boundary, beta, t_e, t_m, sigma_e, gamma, dc=1.0, dt=.005):
...
def diffusion_condition(params, n_trials):
    ...
    for i in range(n_trials):
        choicert[i], z[i] = diffusion_trial(drift, boundary, beta, t_e, t_m, sigma_e, gamma)
    return choicert, z
...
trainer = ParameterEstimationTrainer(
    ...
    checkpoint_path="../checkpoint/N200_nonsingle_trial_sigma_gamma"
)
...
true_vs_estimated(true_params, param_means, ['drift', 'boundary', 'beta', 't_e', 't_m', 'sigma_e', 'gamma'], filename="../true_vs_estimate/N200_nonsingle_trial_sigma_gamma")
