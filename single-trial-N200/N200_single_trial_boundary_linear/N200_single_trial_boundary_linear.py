...
def prior(batch_size):
    ...
    # a_slope ~ U(.05, .9)
    n_parameters = 8
    p_samples = np.random.uniform(low=(-3.0, 0.5, 0.1, 0.05, 0.06, 0.0, 0.0, .01),
                                  high=(3.0, 3.0, 0.9, 0.6,  0.8,  0.3, 0.3, .5), size=(batch_size, n_parameters))
...
trainer = ParameterEstimationTrainer(
    ...
    checkpoint_path="../checkpoint/N200_single_trial_boundary_linear"
)
...
true_vs_estimated(true_params, param_means, ['drift', 'boundary', 'beta', 'mu_tau_e', 'tau_m', 'sigma', 'varsigma', 'a_slope'], filename="../true_vs_estimate/N200_single_trial_boundary_linear")
